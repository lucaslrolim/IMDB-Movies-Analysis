...
**EXPLICAR O QUE SÃO AS MEDIDAS MIN, 1st Qu, Median, 3rd Qu. E MAX QUE SÃO EXIBIDAS**
...
**COMENTAR MAIS SOBRE A MATRIZ DE CORRELAÇÃO. ORDENAR PELO MENOS AS 5 VARIÁVEIS MAIS CORRELACIONADAS E COMENTAR ALGUMA CONCLUSÃO QUE VOCÊ CHEGOU**.
...
**ADICIONAR CONCUSÃO A ESSE GRÁFICO**
...
directorMovies = sapply(trainData$director_name,directorMNumber)
trainData$directorIMDB <- as.numeric(directorIMDB)
trainData$directorMovies <- as.numeric(directorMovies)


directorHistory = summarise(group_by(testData,director_name),mean=mean(imdb_score),number = n())
directorIMDB_t =  sapply(testData$director_name,directorMean)
directorMovies_t = sapply(testData$director_name,directorMNumber)
testData$directorIMDB <- as.numeric(directorIMDB_t)
testData$directorMovies <- as.numeric(directorMovies_t)

trainData = subset(trainData, select=-c(director_name))
testData = subset(testData, select=-c(director_name))
