...
**Em primeiro lugar vamos criar um data.frame para armazenar nossos resultados**
...
results <- rbind(results,data.frame(Nome = "Regressão Linear",
                                      RMSE = sqrt(SSE / nrow(testData)),
                                      MSE = SSE / nrow(testData))
                   )
results
...
results <- rbind(results,data.frame(Nome = "Floresta Aleatória",
                                      RMSE = min(RMSE_vector),
                                      MSE = min(RMSE_vector)*min(RMSE_vector))
                   )
results
```
...
```{r RedeNeural}
n <- names(trainData)
nn <- neuralnet(model_variables,data= trainData,hidden=c(5,3),linear.output=T)
