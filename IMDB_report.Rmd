...
Os dados utilizados nessa análise foram minerados por Chuan Sun no site do IMDB e depois disponibilizados na plataforma Kaggle.Tanto o tópico desse conjunto de dados quanto uma explicação em detalhes sobre o processo de mineração podem ser encontrados nos seguintes links:

- [Predict Movie Rating - NYC SCIENCE ACADEMY](https://blog.nycdatascience.com/student-works/machine-learning/movie-rating-prediction/)
...
Por fim, a linguagem R foi escolhida por ser a que possui bibliotecas no estado da arte no que tange a tratamentos estatísticos e criação de modelos de regressão, classificação e agrupamentos. Essa linguagem é uma "sucessora" da linguagem S e conta com a ampla adoção acadêmica e diveras bibliotecas otimizadas.
...
Nesse ponto é importante notar que com a exclusão de linhas que entradas que possuiam valores ausentes, nosso conjunto de dados foi reduzido para um universo de 3801 amostras. Ou seja, **2059 registros possuiam algum valor ausente**. 
...
```
**Variáveis com forte correlação positiva:**

cast_total_facebook_likes e actor_1_facebook_likes: 0,94
movie_facebook_likes e num_critic_for_reviews: 0,70
gross e num_voted_users: 0,63
...
```{r HistogramasTransformação}

## num_critic_for_reviews

ggplot(imdb_data,aes(x=num_critic_for_reviews))+ geom_histogram() + xlab("num_critic_for_reviews") + ylab("Número de Filmes")
ggplot(imdb_data,aes(x=log(num_critic_for_reviews)))+ geom_histogram() + xlab("log(num_critic_for_reviews)") + ylab("Número de Filmes")



## Gross

ggplot(imdb_data,aes(x=gross))+ geom_histogram() + xlab("gross") + ylab("Número de Filmes")

ggplot(imdb_data,aes(x=log(gross)))+ geom_histogram() + xlab("log(gross)") + ylab("Número de Filmes")



## num_voted_users

ggplot(imdb_data,aes(x=num_voted_users))+ geom_histogram() + xlab("num_voted_users") + ylab("Número de Filmes")

ggplot(imdb_data,aes(x=log(num_voted_users)))+ geom_histogram() + xlab("log(num_voted_users)") + ylab("Número de Filmes")


## num_user_for_reviews

ggplot(imdb_data,aes(x=num_user_for_reviews))+ geom_histogram() + xlab("num_user_for_reviews") + ylab("Número de Filmes")

ggplot(imdb_data,aes(x=log(num_user_for_reviews)))+ geom_histogram() + xlab("log(num_user_for_reviews)") + ylab("Número de Filmes")
...
```{r boxplot}
boxplot(imdb_data$num_critic_for_reviews, ylab="num_critic_for_reviews")
boxplot(imdb_data$movie_facebook_likes, ylab="movie_facebook_likes")
boxplot(imdb_data$duration, ylab="duration")
boxplot(imdb_data$director_facebook_likes, ylab="director_facebook_likes")
boxplot(imdb_data$actor_3_facebook_likes, ylab="actor_3_facebook_likes")
boxplot(imdb_data$actor_2_facebook_likes, ylab="actor_2_facebook_likes")
boxplot(imdb_data$actor_1_facebook_likes, ylab="actor_1_facebook_likes")
boxplot(imdb_data$gross, ylab="gross")
boxplot(imdb_data$num_voted_users, ylab="num_voted_users")
boxplot(imdb_data$cast_total_facebook_likes, ylab="cast_total_facebook_likes")
boxplot(imdb_data$facenumber_in_poster, ylab="facenumber_in_poster")
boxplot(imdb_data$num_user_for_reviews, ylab="num_user_for_reviews")
boxplot(imdb_data$budget, ylab="budget")
boxplot(imdb_data$imdb_score, ylab="imdb_score")
boxplot(imdb_data$aspect_ratio, ylab="aspect_ratio")
```
**A FAZER: ENCONTRAR OUTLIERS E COMENTAR SOBRE**
...
```{r IMDBvsDiretor}
ggplot(directorHistory,aes(x=number, y=mean)) + geom_point(colour="grey60") +
...
RMSE
MSE = mean((testData$imdb_score - DumbPredict)^2)
```
...
**A FAZER: EXPLICAR EM MAIS DETALHES, COMO É SOLICITADO NA PÁGINA 5 DA AVALIAÇÃO**
